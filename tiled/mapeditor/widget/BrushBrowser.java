...
public class BrushBrowser extends JPanel {
	...
	private void init() {
		...
		for(int i=0; i<iters; i++) {						//Use Fibonacci to create a sampling of brushes...
			int dim = n+n1;
			int hold=n1;
			n1=dim;
			n=hold;
	...
	public void paint(Graphics g) {
		...
		if(brushes.indexOf(selectedBrush)>0) {
			...
			int bx =  index % (this.getPreferredSize().width/maxWidth);
			int by =  index / (this.getPreferredSize().width/maxWidth);
